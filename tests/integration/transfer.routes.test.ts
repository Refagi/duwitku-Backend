...
async function createTransferViaApi(
  token: string,
  body: { fromAccountId: string; toAccountId: string; amount: number; date?: string; note?: string },
) {
...
describe("Transfer routes", () => {
  ...
  describe("GET /v1/transfers", () => {
    ...
    it("200 pencarian note (q) case-insensitive", async () => {
      ...
      const match = await insertTransfer(user.id, from.id, to.id, { note: "Transfer buat bayar kos" });
