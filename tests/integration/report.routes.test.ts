...
describe("Report routes", () => {
  ...
  describe("GET /v1/reports/summary", () => {
    ...
    it("200 categoryBreakdown persentase dihitung benar dan total mendekati 100%", async () => {
      ...
      const body = await res.json();
      const makanBreakdown = body.data.categoryBreakdown.find((b: any) => b.categoryName === "Makan");
      const transportBreakdown = body.data.categoryBreakdown.find((b: any) => b.categoryName === "Transport");
      ...
      const totalPercentage = body.data.categoryBreakdown.reduce((sum: number, b: any) => sum + b.percentage, 0);
