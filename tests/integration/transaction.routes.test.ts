...
describe('Transaction routes', () => {
  ...
  describe('GET /v1/transactions', () => {
    ...
    it('200 filter berdasarkan accountId, categoryId, dan type', async () => {
      ...
      const res = await app.request(`/v1/transactions?accountId=${accountA.id}&type=INCOME`, {
    ...
    it('200 filter berdasarkan rentang tanggal (from/to)', async () => {
      ...
      const inRange = await insertTransaction(user.id, account.id, category.id, {
        date: new Date('2026-03-15'),
      })
      await insertTransaction(user.id, account.id, category.id, { date: new Date('2026-06-01') })

      const res = await app.request('/v1/transactions?from=2026-03-01&to=2026-03-31', {
        headers: authCookieHeader(token),
      })
    ...
    it('200 filter berdasarkan rentang nominal (minAmount/maxAmount)', async () => {
      ...
      const res = await app.request('/v1/transactions?minAmount=20000&maxAmount=100000', {
        headers: authCookieHeader(token),
      })
    ...
    it('200 pencarian note (q) case-insensitive', async () => {
      ...
      const match = await insertTransaction(user.id, account.id, category.id, {
        note: 'Beli Kopi Susu',
      })
    ...
    it('200 pagination page & limit', async () => {
      ...
      const res = await app.request('/v1/transactions?page=2&limit=2', {
        headers: authCookieHeader(token),
      })
