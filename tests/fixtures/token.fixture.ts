...
export async function insertRefreshToken(userId: string, overrides: Partial<{ expires: dayjs.Dayjs; blacklisted: boolean }> = {}) {
...
export async function insertExpiredRefreshToken(userId: string) {
  ...
  return prisma.token.create({
    data: { token, userId, expires: expires.toDate(), type: TokenTypes.REFRESH, blacklisted: false },
  });
}